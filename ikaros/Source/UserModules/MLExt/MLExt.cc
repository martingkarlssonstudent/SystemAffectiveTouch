...
Module *
MLExt::Create(Parameter * p)
{
	return new MLExt(p);
}

MLExt::MLExt(Parameter * p): Module(p)
{
    CheckParameters();
}
...
void MLExt::Init()
{
	...
	touchIdInput_array = GetInputArray("TOUCHID_INPUT");
	touchIdInput_NoCols = GetInputSize("TOUCHID_INPUT");

	touchCertainty_matrix = GetOutputMatrix("TOUCHCERTAINTY_OUTPUT");
	touchCertainty_NoRows = GetOutputSizeX("TOUCHCERTAINTY_OUTPUT");

	MLExtOutput_array = GetOutputArray("MLExt_OUTPUT");

	touchActiveInput_array = GetInputArray("TOUCHACTIVE_INPUT");
...
Status tf_ReadLabelsFile(const string& file_name, std::vector<string>* result,
...
static Status tf_ReadEntireFile(tensorflow::Env* env, const string& filename,
                             Tensor* output) {
...
Status tf_ReadTensorFromImageFile(const string& file_name, const int input_height,
  ...
  TF_RETURN_IF_ERROR(
      tf_ReadEntireFile(tensorflow::Env::Default(), file_name, &input));
...
// can use to run it.
Status tf_LoadGraph(const string& graph_file_name,
                 std::unique_ptr<tensorflow::Session>* session) {
...
Status tf_GetTopLabels(const std::vector<Tensor>& outputs, int how_many_labels,
...
Status tf_PrintTopLabels(const std::vector<Tensor>& outputs,
  ...
  Status read_labels_status =
      tf_ReadLabelsFile(labels_file_name, &labels, &label_count);
  ...
  Tensor indices;
  Tensor scores;
  TF_RETURN_IF_ERROR(tf_GetTopLabels(outputs, how_many_labels, &indices, &scores));
...
// one that's expected.
Status tf_CheckTopLabel(const std::vector<Tensor>& outputs, int expected,
                     bool* is_expected) {
  *is_expected = false;
  Tensor indices;
  Tensor scores;
...
int tf_rungraph(int argcIn, char* argvIn[], float * touchIdInput_array_local, int nocols_touchIdInput_array, int norows_touchIdInput_array, float ** touchCertainty_matrix_local, int noTouchTypesOut)
{
...
//	string graph = "/home/martin/tfgraphs/sosy_dnnclassifier/saved_model.pb";
//	string graph = "/home/martin/tfgraphs/frozen_graph_SosySLTouch_In2400.pb";
	string graph = "/home/martin/tfgraphs/frozen_graph_SosySLTouch_ALL.pb";
...
//  string labels = "/home/martin/tensorflow_cc/example_SosyANN/labels.txt";
  string labels = "/home/martin/tfgraphs/labels.txt";
...
//  string output_layer = "sequential_1/dense_5/Softmax";
...
//	std::cout << "Init model";
  ...
  Status load_graph_status = tf_LoadGraph(graph_path, &session);
  if (!load_graph_status.ok()) {
...
//	std::cout << '\n';
	tensorflow::Tensor inputTensor(tensorflow::DT_FLOAT, tensorflow::TensorShape({norows_touchIdInput_array, nocols_touchIdInput_array}));
	auto input_map = inputTensor.tensor<float, 2>();
	for (int i=0; i<norows_touchIdInput_array; i++)
	{
		for (int j=0; j<nocols_touchIdInput_array; j++)
		{
//			cout <<  touchIdInput_array_local[i][j] << " ";
			input_map(i,j) = touchIdInput_array_local[j];
		}
//		std::cout << '\n';
	}
...
/*	Status train_status = session->Run({{resized_tensor, resized_tensor},
  ...
  if (self_test) {
    ...
    Status check_status = tf_CheckTopLabel(outputs, 653, &expected_matches);
    if (!check_status.ok()) {
  ...
  Status print_status = tf_PrintTopLabels(outputs, labels, touchCertainty_matrix_local, noTouchTypesOut);
  if (!print_status.ok()) {
...
void MLExt::Tick()
{
	...
	if (touchActiveInput_array [0] > 0)
	{
		...
		{
			touchCertainty_matrix[0][i] = 0;
		}

		tf_rungraph(1, argvdefault, touchIdInput_array, touchIdInput_NoCols, 1, touchCertainty_matrix, noTouchTypesOut);

		cout << "MLExt touchCertainty matrix";
		std::cout << '\n';
		cout << "touchCertainty_NoRows: " << touchCertainty_NoRows;
		std::cout << '\n';
		for (int i=0; i<touchCertainty_NoRows; i++)
		{
				if (touchCertainty_matrix[0][i] < touchCertaintyCutoff)
				{
					touchCertainty_matrix[0][i] = 0;
				}
...
void MLExt::CheckParameters()
{
}

static InitClass init("MLExt", &MLExt::Create, "Source/UserModules/MLExt/");
